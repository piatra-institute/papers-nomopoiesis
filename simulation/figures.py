...
def plot_selection(results: dict, path: str) -> None:
    ...
    a1.set_title("the niche-mediated term reverses the sign", fontsize=10, color=INK)
    ...
    a2.set_title("ecological inheritance sets a threshold", fontsize=10, color=INK)
    for ax in (a1, a2):
        _style(ax)
    fig.tight_layout()
...
def plot_gain(results: dict, path: str) -> None:
    ...
    ax.plot(k, sg, color=NICHE, lw=1.9, label="genuine (state edits the law)")
    ax.plot(k, sf, color=DIRECT, lw=1.9, ls="--", label="forced (external clock)")
    ax.set_yscale("log")
    ax.set_xlabel("time steps after a one-off construction action")
    ax.set_ylabel("trajectory separation from counterfactual")
    ax.set_title("nomopoietic gain grows only when the state edits the law",
                 fontsize=10, color=INK)
    ax.annotate(rf"$G_N(40)={ng['gain_genuine_k40']:.1f}$ genuine vs "
                rf"${ng['gain_forced_k40']:.2f}$ forced",
...
def plot_game(results: dict, path: str) -> None:
    ...
    ax.axhline(gr["fixed_game_low_env_fixation"], color=DIRECT, lw=1.1, ls="--",
               label="every frozen game fixates")
    ax.set_xlabel("time (coarse steps)")
    ax.set_ylabel("frequency / state")
    ax.set_ylim(-0.05, 1.08)
    ax.set_title("play rewrites the matrix: oscillation the frozen game forbids",
                 fontsize=10, color=INK)
